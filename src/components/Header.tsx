          ...
          {isOpen && (
            <div className="absolute top-full right-0 mt-2 w-48 sm:w-52 bg-[#12171D] rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.8)] border border-gray-700/90 overflow-hidden z-[500] animate-in fade-in zoom-in-95 duration-150">
              ...
              <button
                ...
                className={cn(
                  "w-full text-left px-3 py-3 hover:bg-gray-800 transition flex items-center justify-between group cursor-pointer",
                  accountType === 'Demo Account' ? "bg-blue-600/20" : ""
                )}
              >
                <div className="flex items-center space-x-2">
                  <div className={cn("w-2 h-2 rounded-full", accountType === 'Demo Account' ? "bg-blue-400" : "bg-gray-600 group-hover:bg-gray-400")} />
                  <span className={cn("text-xs sm:text-sm font-bold", accountType === 'Demo Account' ? "text-white" : "text-gray-400 group-hover:text-white")}>Demo Account</span>
                </div>
                {accountType === 'Demo Account' && <div className="w-1.5 h-1.5 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.6)]" />}
              </button>
              <button
              ...
              >
                <div className="flex items-center space-x-2">
                  <div className={cn("w-2 h-2 rounded-full", accountType === 'Real Account' ? "bg-green-400" : "bg-gray-600 group-hover:bg-gray-400")} />
                  <span className={cn("text-xs sm:text-sm font-bold", accountType === 'Real Account' ? "text-white" : "text-gray-400 group-hover:text-white")}>Real Account</span>
                </div>
                {accountType === 'Real Account' && <div className="w-1.5 h-1.5 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]" />}
              </button>
            </div>
          )}
        </div>
