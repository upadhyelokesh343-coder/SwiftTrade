...
import { collection, query, onSnapshot, doc, updateDoc, setDoc, orderBy, limit, getDocs, arrayUnion, addDoc, getDoc, Timestamp } from 'firebase/firestore';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onClose }) => {
  ...
  const handleApproveDeposit = async (dep: AdminDeposit) => {
    try {
      ...
      // 2. Add real balance to user's account in Firestore
      if (dep.userId) {
        const userRef = doc(db, 'users', dep.userId);
        const userSnap = await getDoc(userRef);
        if (userSnap.exists()) {
          const currentReal = userSnap.data().realBalance || 0;
          await updateDoc(userRef, { realBalance: currentReal + dep.amountUSD });
        }
      }
      ...
      await addDoc(collection(db, 'notifications'), {
        ...
        message: `Your deposit of ₹${dep.amountINR.toLocaleString()} ($${dep.amountUSD}) via ${dep.paymentMethod.toUpperCase()} has been approved and credited to your Real Balance!`,
        read: false,
  ...
  const handleRejectWithdrawal = async (withItem: AdminWithdrawal) => {
    try {
      ...
      if (withItem.userId) {
        ...
        if (userSnap.exists()) {
          const currentReal = userSnap.data().realBalance || 0;
          await updateDoc(userRef, { realBalance: currentReal + withItem.amount });
        }
      }
  ...
  const handleUpdateBalance = async (userId: string, type: 'demo' | 'real', amount: number) => {
    try {
      const userRef = doc(db, 'users', userId);
      await updateDoc(userRef, {
        [type === 'demo' ? 'demoBalance' : 'realBalance']: amount,
        updatedAt: new Date()
      });
      setEditingUser(null);
    } catch (error) {
                ...
                {deposits.map((dep) => (
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-white/10">
                      <div className="flex items-center space-x-3">
                        <div className="w-12 h-12 rounded-2xl bg-green-500/20 border border-green-500/30 flex items-center justify-center text-green-400 font-black text-lg">
                          ₹
                        </div>
                        <div>
                          <div className="flex items-center space-x-2">
                          ...
                          <p className="text-xs text-gray-400 mt-0.5 font-mono">
                            UTR / Ref: <span className="text-amber-400 font-bold select-all">{dep.utrNumber || 'N/A'}</span>
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-4">
                        <div className="text-right">
                          <div className="text-xl font-black text-green-400">₹{dep.amountINR?.toLocaleString() || 0}</div>
                          <div className="text-xs text-gray-400 font-mono font-bold">≈ ${dep.amountUSD || 0} USD</div>
                        </div>
            ...
            >
              ...
              <div className="space-y-8">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        onChange={(e) => setEditingUser({...editingUser, demoBalance: parseFloat(e.target.value)})}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        onChange={(e) => setEditingUser({...editingUser, realBalance: parseFloat(e.target.value)})}
                ...
                <div className="flex gap-4 pt-6">
                  ...
                  <button 
                    onClick={() => {
                      handleUpdateBalance(editingUser.id, 'demo', editingUser.demoBalance);
                      handleUpdateBalance(editingUser.id, 'real', editingUser.realBalance);
                    }}
