...
import { CryptomusDepositModal } from './components/CryptomusDepositModal';
...
export default function App() {
  ...
  // Cryptomus Gateway Deposit Modal State
  ...
  return (
    <div className="flex flex-col h-full h-[100dvh] bg-[#0B0E11] text-gray-200 font-sans overflow-hidden select-none">
      <Header 
        ...
        onOpenDeposit={() => setIsCryptomusModalOpen(true)}
      />
      ...
      <main className="flex flex-col lg:flex-row flex-1 relative overflow-hidden z-10 w-full max-w-full min-h-0 pb-14 lg:pb-0">
        {activeTab === 'history' ? (
          ...
          <WithdrawalPage 
            ...
            realBalance={realBalance}
            demoBalance={demoBalance}
            onSwitchToReal={() => setAccountType('Real Account')}
            onOpenDeposit={() => setIsCryptomusModalOpen(true)}
            onBackToTrade={() => setActiveTab('trade')} 
          />
        ) : activeTab === 'deposit' ? (
      ...
      {/* Cryptomus Gateway Deposit Modal */}
